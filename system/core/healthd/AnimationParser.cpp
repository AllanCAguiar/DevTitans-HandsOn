...
bool parse_animation_desc(const std::string& content, animation* anim) {
    ...
    static constexpr const char* message_prefix = "message_display";
    ...
    for (const auto& line : base::Split(content, "\n")) {
        ...
        } else if (remove_prefix(line, message_prefix, &rest)) {
            if (!parse_text_field(rest, &anim->text_message)) {
                LOGE("Bad message_display format: %s\n", line.c_str());
                return false;
            }
