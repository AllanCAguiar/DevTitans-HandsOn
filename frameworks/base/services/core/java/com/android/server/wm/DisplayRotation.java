...
public class DisplayRotation {
    ...
    private final ContextualRotationController mContextualController; // nova variavel para rotacao contextual
    ...
    DisplayRotation(WindowManagerService service, DisplayContent displayContent,
        ...
        //inicializacao da rotatacao contextual
        mContextualController = new ContextualRotationController(context, () -> {
            // FORCE UPDATE = TRUE (O primeiro 'true')
            // Isso obriga o sistema a recalcular a rotação AGORA, mesmo se o sensor estiver parado.
            mService.updateRotation(true, false); 
        });
    }
    ...
    private void updateOrientationListenerLw() {
        ...
        if (screenOnEarly
            ...
            // --- INICIO DA MODIFICAÇÃO (Paulo Build) ---
            // Verifica se o nosso controlador detectou mídia ativa
            boolean isContextualActive = isContextualVideoActive();
            
            // A Lógica Mágica:
            // O sensor deve ligar se:
            // 1. O sistema pede (needSensorRunning) OU
            // 2. A rotação está TRAVADA, mas temos VÍDEO tocando (isContextualActive)
            boolean shouldEnableSensor = needSensorRunning() || 
                (isContextualActive && mUserRotationMode == WindowManagerPolicy.USER_ROTATION_LOCKED);

            if (shouldEnableSensor) {
                disable = false;
                // Enable listener if not already enabled.
                if (!mOrientationListener.mEnabled) {
                    mOrientationListener.enable();
                    android.util.Log.v("ContextualRotation", "Sensor de rotacao ativado forcadamente por midia!");
                }
            }
            // --- FIM DA MODIFICAÇÃO ---
        }
    ...
    private boolean needSensorRunning() {
        ...
        // --- MODIFICAÇÃO PAULO (SENSOR FORCE) ---
        // Se detectarmos vídeo, OBRIGAMOS o sensor a ligar, 
        // ignorando se a rotação automática está desligada.
        if (isContextualVideoActive()) {
            return true;
        }
        // --- FIM ---
    ...
    int rotationForOrientation(@ScreenOrientation int orientation,
        ...
        if (!isDefaultDisplay) {
            ...
            // --- INICIO DA LÓGICA CONTEXTUAL ---
            boolean isContextualMode = isContextualVideoActive();

            if (isContextualMode) {
                // Se tem vídeo/áudio, tenta usar o sensor (comportamento de rotação livre)
                boolean allowed = true;
                if (orientation != ActivityInfo.SCREEN_ORIENTATION_FULL_SENSOR
                        && orientation != ActivityInfo.SCREEN_ORIENTATION_FULL_USER) {
                    allowed = RotationPolicy.isRotationAllowed(sensorRotation, mUserRotationAngles,
                ...
                if (sensorRotation != -1) {
                     android.util.Log.v("ContextualRotation", "Forcando rotacao (Media Ativa): " + sensorRotation);
                }
            } else {
                // Se NÃO tem vídeo, respeita a trava do usuário (Linha original)
                preferredRotation = mUserRotation;
            }
            // --- FIM DA LÓGICA ---
    ...
    private class OrientationListener extends WindowOrientationListener implements Runnable {
        ...
        public void onProposedRotationChanged(@Surface.Rotation int rotation) {
            ...
            // --- MODIFICAÇÃO PAULO (SMART ROTATION V4) ---
            
            // CORREÇÃO: Usamos "DisplayRotation.this" para chamar o método da classe Pai
            if (DisplayRotation.this.isContextualVideoActive()) {
                android.util.Log.v("ContextualRotation", "MATCH! Video detectado via Helper. Girando...");
                
                // Usamos "DisplayRotation.this" para acessar as variáveis da classe Pai
                DisplayRotation.this.mRotationChoiceShownToUserForConfirmation = ROTATION_UNDEFINED;
